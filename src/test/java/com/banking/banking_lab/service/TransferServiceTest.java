...
class TransferServiceTest {
	...
	@Test
	void transfer_should_lock_accounts_in_order() {

		Account from = new Account("222", BigDecimal.valueOf(1000));

		Account to = new Account("111", BigDecimal.ZERO);
		...
		when(idempotencyRepository.findByKey("abc")).thenReturn(Optional.empty());

		// quan trọng:
		// id nhỏ phải lock trước

		when(accountRepository.findByIdForUpdate(1L)).thenReturn(Optional.of(to));

		when(accountRepository.findByIdForUpdate(2L)).thenReturn(Optional.of(from));
	...
	void transfer_account_not_found() {
		...
		assertThrows(AccountNotFoundException.class, () -> transferService.transfer("abc", request));
